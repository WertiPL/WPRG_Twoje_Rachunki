...
<form id="selectform" name="selectSomething" action="" method="GET" class="form-group">
    <label for="bdaymonth">Stats by (month and year):</label>
...
echo $month;
          ...
          //  SELECT * FROM `bills` WHERE `DATA` like "2022-06-%";
...
$nick2="User2";
            ...
            $drugiWydal= "SELECT SUM(kwota) as `Suma`  FROM `$tableName` WHERE `ID` != 1 AND `who`=\"User2\";";
                    if($PayForBill==$nick1)
                    {
                        $Wydal1+=$HowManyPaid;
                            ...
                            //$tempZap +=$SumaIledrugiMaNaRachunkuPierwszego;
        ...
        <h4><?php echo $nick1." Zapłacił za rachunki w tym miesiącu ".$Wydal1." Musi oddac jeszcze ".($oddaj1)  ?></h4>


            <h4><?php echo $nick2." Zapłacił za rachunki w tym miesiącu ".$Wydal2." Musi oddac jeszcze ".($oddaj2)  ?></h4>
<?php
}
